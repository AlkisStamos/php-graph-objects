...
class GraphAdapterCallbackTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testGraphableFinalizeMap()
    {
        $data = Util::loadData('child');
        $obj = GraphObjectChild::map($data,'test-finalize-callback');
        $this->assertSame('test-finalize-callback', $obj->aString);
        $this->assertSame(5, $obj->anInteger);
        $this->assertSame(false, $obj->aBoolean);
        $this->assertTrue(Util::arrays_are_similar(array('a', 'b', 'c', 'd'), $obj->flatArray));
        $this->assertSame(5.5, $obj->aDouble);
    ...
    public function testGraphableFinalizeInject()
    {
        $data = Util::loadData('child');
        $obj = GraphObjectChild::inject($data,'test-finalize-callback');
        $this->assertSame('test-finalize-callback', $obj->aString);
        $this->assertSame(7, $obj->anInteger);
        $this->assertSame(false, $obj->aBoolean);
        $this->assertTrue(Util::arrays_are_similar(array('a', 'b', 'c', 'd'), $obj->flatArray));
    ...
    public function testGraphableFinalizeEmpty()
    {
        $data = Util::loadData('child');
        $obj = GraphObjectChild::inject($data,'test-finalize-callback');
        $this->assertSame('test-finalize-callback', $obj->aString);
        ...
        $obj->aString = null;
        $obj->anInteger = null;
        $obj->childValueStr = null;
        $obj->mapEmpty($data,'test-finalize-callback');
        $this->assertSame('test-finalize-callback', $obj->aString);
        $this->assertSame(5, $obj->anInteger);
        $this->assertSame(false, $obj->aBoolean);
        $this->assertTrue(Util::arrays_are_similar(array('a', 'b', 'c', 'd'), $obj->flatArray));
